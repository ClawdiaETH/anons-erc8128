...
# Uses manual calldata encoding to avoid cast array parsing issues
set -e
...
echo "📝 Encoding proposal calldata..."
# Get function signature
SIG=$(cast sig "propose(address[],uint256[],string[],bytes[],string)")
echo "Function selector: $SIG"

# Encode the arguments
ENCODED=$(cast abi-encode "propose(address[],uint256[],string[],bytes[],string)" \
  "[$TARGET]" \
  "[0]" \
  '[""]' \
  "[0x]" \
...
# Combine selector + arguments
CALLDATA="${SIG}${ENCODED:2}"
...
# Use cast send with selector + encoded data
cast send "$GOVERNOR" \
  "$CALLDATA" \
  --private-key "$KEY" \
  --rpc-url "$RPC_URL"
